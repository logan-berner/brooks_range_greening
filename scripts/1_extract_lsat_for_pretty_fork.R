...
transect.pts <- st_read('gis_data/PrettyFork.shp')
...
aoi.r <- raster(xmn = min(coords[,1]-100), xmx = max(coords[,1]+100),
       ymn = min(coords[,2]-100), ymx = max(coords[,2]+100), resolution = 50, crs = ak.aea.proj)
aoi.r[] <- 1:ncell(aoi.r)
...
transect.pts <- transect.pts %>% st_sf %>% mutate(site = paste0('pt', 1:nrow(st_coordinates(transect.pts))), 
                                           region = c("pretty_fork"))

# transect.pts <- transect.pts[,c(6,7)]

# write out 
st_write(transect.pts, dsn = 'gis_data/PrettyFork_Sample45mGrid.shp', append = F)

# Extract Landsat time-series for points along transect
ee_Initialize()
lsat_export_ts(transect.pts, file_prefix = 'lsat_pretty_fork', drive_export_dir = 'earth_engine', 
